...
const HeroSection = () => {
  return (
    <section className="relative min-h-[92vh] sm:min-h-screen flex items-center pt-28 sm:pt-32 overflow-hidden">
      {/* Background Decorations - pointer-events-none added to prevent click-blocking */}
      ...
      <div className="absolute top-20 right-0 w-[500px] h-[500px] rounded-full bg-accent/5 blur-3xl -z-10 pointer-events-none" />
      <div className="absolute bottom-0 left-0 w-[400px] h-[400px] rounded-full bg-primary/5 blur-3xl -z-10 pointer-events-none" />

      <div className="max-w-7xl mx-auto px-5 sm:px-6 lg:px-12 w-full relative z-10">
        <div className="grid lg:grid-cols-2 gap-10 lg:gap-20 items-center">
          <div>
            ...
            <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.4 }} className="mt-10 flex flex-wrap items-center gap-4 relative z-20">
              {/* Internal Link for Questionnaire */}
              <Link 
                to="/questionnaire" 
                className="group inline-flex items-center gap-2.5 px-8 py-4 rounded-full bg-primary text-primary-foreground font-bold text-sm tracking-wide hover:bg-dark-blue-light transition-all duration-400 shadow-xl shadow-primary/20 hover:shadow-2xl hover:shadow-primary/30 hover:-translate-y-1 cursor-pointer"
              >
                Get Started
              ...
              {/* External Link for Tutor Application */}
              <a
                href="https://www.cognitoforms.com/BPanaceaLimited/BPanaceaLimitedTutorApplication"
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-2 px-8 py-4 rounded-full border-2 border-primary/15 text-primary font-bold text-sm tracking-wide hover:border-primary/40 hover:bg-primary/5 transition-all duration-300 cursor-pointer"
              >
                Register for Work
              </a>
            </motion.div>
          ...
          <motion.div initial={{ opacity: 0, scale: 0.92 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.9, delay: 0.2 }} className="relative">
            <div className="relative rounded-3xl overflow-hidden shadow-2xl shadow-primary/15">
              <img src={heroBg} alt="UK tutor helping a school pupil" className="w-full h-[300px] sm:h-[400px] lg:h-[540px] object-cover" width={1280} height={960} />
